...
export default function SupportClient() {
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'chat' ? (
        <section className="overflow-hidden rounded-[28px] border border-[#e6e8ec] bg-white shadow-[0_16px_32px_rgba(26,26,24,0.04)]">
          <div className="border-b border-[#eee7dc] px-4 py-4">
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#64748b]">Assistant</p>
            <h2 className="mt-2 text-lg font-semibold text-[#0f172a]">Ask a workflow question</h2>
          </div>
          ...
          <div className="flex h-[58vh] flex-col">
            <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
              ...
              {loading ? (
                <div className="flex justify-start">
                  <div className="flex gap-1 rounded-[22px] bg-[#f7f8f9] px-4 py-3">
                    {[0, 1, 2].map(index => (
            ...
            {messages.length === 1 ? (
              <div className="border-t border-[#eee7dc] px-4 py-3">
                <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#64748b]">Quick prompts</p>
                <div className="flex flex-wrap gap-2">
                  {QUICK_PROMPTS.map(prompt => (
                    <button
                      key={prompt}
                      onClick={() => sendMessage(prompt)}
                      className="rounded-full bg-[#f7f8f9] px-3 py-1.5 text-xs font-medium text-[#0f172a]"
                    >
                      {prompt}
                    </button>
                  ))}
            ...
            <div className="border-t border-[#eee7dc] p-4">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-2xl border border-[#e6e8ec] bg-[#fafbfc] px-4 py-3 text-sm text-[#0f172a] placeholder:text-[#9c978d] focus:outline-none"
                />
                <button
                  onClick={() => sendMessage()}
                  disabled={!input.trim() || loading}
                  className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#6B2C91] text-[#0f172a] disabled:opacity-40"
                >
        ...
        <section className="space-y-3">
          {[
            ...
            <article key={item.label} className="rounded-[24px] border border-[#e6e8ec] bg-white p-4 shadow-[0_12px_26px_rgba(23,23,22,0.04)]">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {item.href ? (
                    <a href={item.href} className="mt-1 inline-block text-sm font-semibold text-[#0f172a] hover:underline">{item.value}</a>
                  ) : (
                    <p className="mt-1 text-sm text-[#0f172a]">{item.value}</p>
                  )}
                </div>
              </div>
            </article>
